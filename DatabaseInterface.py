...
if __name__ == "__main__":
    ...
    # insertRow(df1, [25, 777, 5])
    print df1.head()
    print df1[df1['user_id'] == 1]['item_id'].tolist()
    # rating_mean = df1.groupby(['item_id'])['rating'].mean()
    # print rating_mean[rating_mean>=4]
    # print rating_mean.head()
    # count = df1.groupby(['item_id'])['user_id'].count().rename('count')
    # result = pd.concat([rating_mean, count], axis=1)
    # print result[result['rating']>4][result[result['rating']>4]['count']>100].index.tolist()
    # item_ids = [3, 1, 5, 6, 7, 333]
    # print rating_mean.loc[item_ids]
    # df2 = connector.connTable["user_activity"]
    # print df2
    # df3 = connector.connTable["item_feature"]
    # print df3.values
    # print df3[266:267].index.tolist()[0]
    # df4 = connector.connTable["user_feature"]
    # print df4.loc[:,"age":]
    # print df1.loc[:,"user_id"]==2
    # print df1[df1.loc[:,"user_id"]==2]
    #print set(df1[df1.loc[:,"user_id"]==2].loc[:,"item_id"]) ##user 2 evaluate items

    # df5 = connector.connTable['inventory']
    # item_ids = [3, 1, 5, 6, 7, 333]
    # print df5.loc[[3]]
    # print connector.extract(DatabaseInterface.INVENTORY_KEY).loc[item_ids].sort_index()